...
public class AESkey {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < roundKeys.length; i += KEY_SIZE) {
            byte[] roundKey = Arrays.copyOfRange(roundKeys, i, i + KEY_SIZE);
            System.out.println(Arrays.toString(roundKey));
        }
    }

    private static byte[] generateRoundKeys(String key) {
        byte[] roundKeys = new byte[KEY_SIZE * 11]; // AES-128 has 11 round keys

        // Convert key to bytes
        byte[] keyBytes = key.getBytes();

        // Copy the original key to the first round key
        System.arraycopy(keyBytes, 0, roundKeys, 0, KEY_SIZE);

        // Generate the remaining round keys
        for (int i = KEY_SIZE; i < roundKeys.length; i += KEY_SIZE) {
            byte[] prevRoundKey = Arrays.copyOfRange(roundKeys, i - KEY_SIZE, i);

            // Perform key schedule operation
            if (i % KEY_SIZE == 0) {
                byte[] temp = Arrays.copyOf(prevRoundKey, KEY_SIZE);

                // Rotate left
                byte tempByte = temp[0];
                System.arraycopy(temp, 1, temp, 0, KEY_SIZE - 1);
                temp[KEY_SIZE - 1] = tempByte;

                // Substitute bytes using S-Box
                for (int j = 0; j < KEY_SIZE; j++) {
                    temp[j] = substituteByte(temp[j]);
                }

                // XOR with round constant
                temp[0] ^= getRoundConstant(i / KEY_SIZE);
            ...
            for (int j = 0; j < KEY_SIZE; j++) {
                roundKeys[i + j] = (byte) (prevRoundKey[j] ^ roundKeys[i - KEY_SIZE + j]);
            }
        }
    ...
    private static byte substituteByte(byte b) {
        ...
        // You can define your own S-Box here
    ...
    private static byte getRoundConstant(int round) {
        // Get round constant value based on the round number
        // Replace the placeholder implementation with your own logic
        byte[] roundConstants = {
